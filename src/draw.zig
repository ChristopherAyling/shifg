...
pub fn draw_image(screen: *ScreenBuffer, img: image.Image, x0: i32, y0: i32) void {
    ...
    const end_x = @min(x0 + img_w, screen.w);
    const end_y = @min(y0 + img_h, screen.h);
    ...
    while (y < end_y) : (y += 1) {
        ...
        while (x < end_x) : (x += 1) {
            ...
            // draw_pixel(screen, x, y, color, 0);
            screen.setPixel(x, y, color, 0);
        }
    }
}

// pub fn draw_pixel(screen: *screen, x: i32, y: i32, color: u32, id: u32) void {
